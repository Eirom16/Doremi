...
TopTrackRow::TopTrackRow(int rank, const QString &title, const QString &artist,
            int plays, int max_plays, const QString &thumbnail, QWidget *parent)
    : QWidget(parent), title_(title), artist_(artist)
{
...
void TopTrackRow::mousePressEvent(QMouseEvent *event) {
    ...
    if (event->button() == Qt::LeftButton) {
        std::string info = title_.toStdString() + " — " + artist_.toStdString();
        emit clicked(info);
    }
}
...
void StatsView::showEvent(QShowEvent *event) {
    ...
    // Request fresh stats from Rust backend
    on_stats_requested();
}

void StatsView::setStatsData(const QString &total_play_time, int total_plays, int unique_artists,
...
{
    card_time_->setValueText(total_play_time);
    card_plays_->setValue(total_plays);
    card_artists_->setValue(unique_artists);

    // Convert QList to QVector for BarChart
    QVector<int> act;
    for (int v : weekly_activity) act.push_back(v);
    bar_chart_->setData(act);

    buildTopTracks(top_titles, top_artists, top_plays, top_thumbnails);
}

void StatsView::buildTopTracks(const QStringList &titles, const QStringList &artists,
                                const QList<int> &plays, const QStringList &thumbnails)
{
    ...
    int max_plays = plays.isEmpty() ? 0 : plays[0];

    int count = static_cast<int>(std::min({titles.size(), artists.size(), plays.size(), thumbnails.size()}));
    for (int i = 0; i < count; ++i) {
        auto *row = new TopTrackRow(i + 1, titles[i], artists[i], plays[i], max_plays, thumbnails[i], top_tracks_widget_);
